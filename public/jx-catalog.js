/* jx-catalog v20260826a — 去掉自动最新开示；师兄学区置顶 */
window.JX = window.JX || {};
...
JX.fetchCatalog = async (opts) => {
  ...
  const res = await fetch(lite ? '/api/catalog?lite=1&v=20260826a' : '/api/catalog', { cache: 'no-store' });
...
/** 师兄维护的「最新开示」学区始终排在最前，其余保持课表顺序 */
JX.orderXuequ = (xuequ) => {
  const list = Array.isArray(xuequ) ? xuequ.slice() : [];
  list.sort((a, b) => Number(JX.isLatestXuequ(b)) - Number(JX.isLatestXuequ(a)));
  return list;
};
...
JX.renderHomeSections = (catalog, opts = {}) => {
  const listSecs = JX.orderXuequ(JX.getXuequ(catalog) || []);
